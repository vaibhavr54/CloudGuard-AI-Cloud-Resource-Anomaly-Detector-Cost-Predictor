...
# ── Mock predict_single before importing app ──────────────────
...
def mock_engineer_single_row(raw: dict) -> dict:
    """Return a minimal engineered row for testing."""
...
def client():
    ...
    with patch("src.predict.predict_single", mock_predict_single), \
         patch("src.feature_engineer.engineer_single_row",
               mock_engineer_single_row):
...
def test_stream_returns_prediction(client):
    with patch("api.routes.generate_realtime_row") as mock_gen, \
         ...
         patch("api.routes.predict_single", mock_predict_single):
